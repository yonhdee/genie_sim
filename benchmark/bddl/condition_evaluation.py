    ...
    def __init__(self, scope, backend, body, object_map):
        ...
        self.children.append(
            get_predicate_for_token(antecedent[0], backend)(
                scope, backend, antecedent[1:], object_map
            )
        )
        self.children.append(
            get_predicate_for_token(consequent[0], backend)(
                scope, backend, consequent[1:], object_map
            )
        )
...
class HEAD(Expression):
    def __init__(self, scope, backend, body, object_map):
        ...
        subexpression = body
        a = get_predicate_for_token(subexpression[0], backend)
        b = a(scope, backend, subexpression[1:], object_map)
        self.children.append(
            get_predicate_for_token(subexpression[0], backend)(
                scope, backend, subexpression[1:], object_map
            )
