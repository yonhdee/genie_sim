    ...
    if type(tokens) is list and tokens.pop(0) == "define":
        ...
        while tokens:
            ...
            if t == "domain":
                ...
                logger.error("%s is not recognized in domain" % t)
...
def parse_action(group):
    ...
    while group:
        ...
        if t == ":parameters":
            ...
            logger.error("%s is not recognized in action" % t)
    ...
    if isinstance(tokens, list) and tokens.pop(0) == "define":
        ...
        while tokens:
            ...
            if t == "problem":
                ...
                logger.error("%s is not recognized in problem" % t)
...
def gen_natural_language_condition(parsed_condition, indent=0):
    ...
    if isinstance(term, list):
        if any([isinstance(subterm, list) for subterm in term]):
            if term[0] == "and":
                logger.info("indent from and:", indent)
                yield f", and\n".join(
                    [
                        list(
                ...
                yield indent_string + "the following is NOT true:\n" + list(
                    gen_natural_language_condition(term[1], indent=indent + 1)
                )[0]
            elif term[0] == "imply":
...
def remove_bddl_whitespace(
    ...
    if save:
        with open(
            "activity_definitions/parsing_tests/test_app_output_nowhitespace.bddl", "w"
        ) as f:
            f.write(bddl)
...
def construct_full_bddl(
    ...
    :param object_list (string): object list (assumed whitespace added with tabs)
